# `burnish study` — three worked examples
...
The whole point of `study`: extract **structural DNA** (macrostructure + archetypes + type-pairing role + colour anchor + rhythm), **never pixels**. If the user wants a 1:1 copy, they want a different tool. Burnish hands them a diagnosis and lets them keep the bones.
...
Macrostructure stamp:
```css
/* Burnish · macrostructure: Type Specimen · H9 hero knobs: glyph=user-mark, tilt=-2deg, grid=1px-soft
...
Macrostructure stamp:
```css
/* Burnish · macrostructure: Long Document · H5 hero knobs: demo=centered, size=24rem
