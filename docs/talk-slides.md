...
> **About this deck** — this is a general playbook for any AI engineer, not a product pitch. Burnish is mentioned once, on slide 15, as one open-source example. The seven fixes apply to v0, Lovable, Bolt, Cursor, Claude Code, or any agent you're building yourself.
>
> **Visual style** — dark slate `#0b1117`, off-white ink `#e6edf3`, GitHub-cool-blue accent `#58a6ff`. Inter for type, Roboto Mono for code. Every slide chrome-free: no eyebrow labels, no decorative hairlines. Footer is the presenter's logo bottom-left and a page number bottom-right.
>
...
*Speaker: Most of you have built an AI app this year and watched it ship a UI you'd never put on your portfolio. This talk is about why that happens, and seven concrete things that fix it — for any agent you use.*
...
## 15 · Burnish — one example our team open-sourced
...
→ `github.com/Nierowheezy/burnish`

*(Visual: the Burnish OG image — "The best landing pages built with AI" with three real example pages baked in.)*

*Speaker: We packaged the seven fixes into a Claude Code skill and open-sourced it. It's one approach — there are others (Anthropic's frontend-design skill, Taste, Impeccable, others). The principle is the same: stop letting the agent invent the rules. Hand it a system.*
...
*(Visual right: a real QR code linking to the Burnish repo.)*
...
- Footer is two elements only: the presenter's logo bottom-left (32×32), and the slide number bottom-right.
...
17 slides for an 18-minute slot ≈ 60 seconds per slide. The section opener (S05) is a 10-second beat. The system-prompt slide (S13) wants a moment for the speaker to read it out loud. The Burnish slide (S15) is intentionally quiet — read the label, don't dwell.
