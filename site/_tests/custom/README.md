...
Three landing pages produced by the **custom** theme route — the new opt-in branch added to Burnish in v0.6.x. Custom is the route for one-off custom palettes + font pairings tuned to a single brand. **Catalog** (the 16 named themes) is the default; custom fires only when the brief signals creative or unique intent.

These three pages are the worked examples written into [`references/custom-theme.md`](../../../skills/burnish/references/custom-theme.md) § G — rendered as actual HTML so the palette + pairing can be inspected visually.
...
5. **Diversification is theme-route-blind.** Each custom run records its three axis values (paper-band / display-style / accent-hue) in `.burnish/log.json`. The next run rotates against them the same way it does against catalog themes.

See [`references/custom-theme.md`](../../../skills/burnish/references/custom-theme.md) for the full protocol.
