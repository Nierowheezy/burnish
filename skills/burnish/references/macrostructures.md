...
Before picking, check the target codebase for a `/* Burnish · macrostructure: <name> · ... */` comment in any existing CSS file. If you find one, **your pick must be a different macrostructure.** No two consecutive Burnish outputs in the same project share a macrostructure.
...
**Pick one. Then read ONLY that one file** from `references/macrostructures/`. Do not load the whole catalogue. Slugs are stable; the diversification rule reads the `<name>` from the `/* Burnish · macrostructure: <name> · ... */` stamp.
...
2. **Check the codebase** for an existing `/* Burnish · macrostructure: <name> · ... */` stamp. If found, exclude that name from your choices.
...
The goal is not novelty for its own sake. The goal is that two pages Burnish builds for two different briefs *look like different sites, not different colour-swaps of the same template*.
