...
| SaaS landing, manifesto, agency, studio, atmospheric, slow-and-editorial | **Kit-led.** Use Burnish imagery kit (washes, transparent abstracts, ornaments) — see [`assets.md` § Placeholder strategy](assets.md) and [`imagery-kit.md`](imagery-kit.md). |
...
- When the user has attached an image asset (or `.burnish/preflight.json` cached one), use it. Never overwrite with a placeholder.
...
The hierarchy below picks the tier *after* this gate decides imagery is needed at all. Skipping this gate is what produces "blob illustration on every page" outputs — exactly the AI-default Burnish refuses.
...
<header class="hero hero--overflow">
  ...
  <p class="hero__lede">Burnish. A design skill that refuses defaults.</p>
</header>
```
```css
...
This signals to future Burnish runs (and to the audit verb) what was chosen and how. It also lets the user see the inferences in one place and redirect if anything's off.
