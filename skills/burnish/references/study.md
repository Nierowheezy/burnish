...
This file is loaded when the `burnish study` verb runs. It defines the protocol for reading a reference the user supplied — either a screenshot they attached or a URL to a live page — naming what makes it work, and producing a *diagnosis report* the user can accept or amend before any code is built.
...
URL mode trades the rhythm pass for everything else getting more accurate. If rhythm is what the user wants extracted, they should attach a screenshot instead — or alongside the URL, but Burnish still defaults to one source at a time (see the "One screenshot, one diagnosis" rule in § Limits).
...
| --- | --- |
| A paid template marketplace listing (ThemeForest, Gumroad templates, Webflow templates, Framer templates, Notion templates) | Refuse. Suggest: "Tell me what you like about it and I'll build with `burnish default` instead." |
...
| --- | --- |
| `themeforest.net/*`, `templatemonster.com/*`, `themely.com/*` (paid template marketplaces) | Refuse. *"This looks like a template marketplace listing. I won't study it. Tell me what about it you like and I'll build with `burnish default` instead."* |
...
After the schema is filled, map the source to one of Burnish's named themes — but **only as a candidate**. The user may pick a different theme for their build.
...
If two themes are equally close, pick whichever is more *categorically distant* from any previous Burnish output for this user (read the existing CSS for a `/* Burnish · macrostructure: ... */` stamp and avoid that theme's family).
...
```css
/* Burnish · macrostructure: Split Studio · H2 hero knobs: ratio=6/6, right=proof, divider=negative-space
...
1. **Fonts cannot be identified from screenshots reliably.** In image mode, Burnish names *roles* and proposes 1–2 candidates from its canon — visual font ID is wrong half the time on custom or modified faces. In **URL mode** the rule flips: the page's `@font-face`, Google Fonts `<link>`, and `next/font` declarations name the typefaces authoritatively, and the diagnosis can name them. The role still travels into the rebuilt page (Burnish may pick a different specific face from the canon for the user's content); the original name is recorded as a side fact.
...
2. **Add a `## Provenance` block.** Inserted between `## System` and `## Tokens`. Carries: the source mode, the URL (URL mode only) or "image (user-attached)" (image mode), the date of extraction, the attestation answer if any, and a one-line note about confidence:
   ...
   - Image mode: *"Tokens are estimated from source-image colour bands. Fonts are role-based with named candidates from the Burnish canon. Rhythm is from a vision pass on the source."*
3. **Add a `## Notes` block** at the end with the anti-patterns the diagnosis flagged as "do NOT carry over." Future Burnish runs reading the file should see these as part of the system's identity.
...
- Subsequent Burnish runs read `design.md` first; diversification inverts to consistency.
...
- If the user says *"now refactor my existing site to match this DNA"*: hand off to **`burnish redesign`** with the schema attached. Redesign preserves the user's content; study supplied the new shape.
