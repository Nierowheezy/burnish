...
A small set of pre-generated abstract / decorative imagery that any Burnish output can pull from when a brief allows non-photographic imagery. The kit lives at:

```
https://useburnish.vercel.app/imagery/<category>/<file>
```
...
.section--wash::before {
  ...
  position: absolute;
  inset: 0;
  background: url("https://useburnish.vercel.app/imagery/watercolor/watercolor-warm-01.webp") center / cover no-repeat;
  opacity: 0.6;
  z-index: -1;
  pointer-events: none;
}
...
**Post-processing.** Trim, transparent-background where applicable, colour-balance against Burnish's OKLCH palette tokens, save as WebP for size + PNG where alpha matters. Ornaments as inline SVG so they inherit `currentColor`.
