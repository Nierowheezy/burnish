...
Loaded by [`SKILL.md`](../SKILL.md) Step 6 when emitting the design system as portable tokens. Defines the four canonical formats Burnish always writes:
...
## Token taxonomy — Burnish's source of truth

Every Burnish output writes these tokens (or a subset, if the page doesn't use one). The names are the source; every other format is a translation.

| Burnish token | Type | Example value |
| --- | --- | --- |
...
The source. Plain CSS custom properties at `:root`. Every Burnish page CSS imports this file at the top:
...
Tailwind v4 reads CSS variables inside `@theme` and generates utilities (`bg-paper`, `text-ink`, etc.) automatically. The translation from Burnish tokens is direct — same variable names, same OKLCH values:
...
@theme {
  ...
  /* Spacing — Tailwind reads --spacing-* by default; we keep Burnish's --space-* names alongside */
  --spacing-3xs: 0.25rem;
  --spacing-2xs: 0.5rem;
  --spacing-xs:  0.75rem;
  --spacing-sm:  1rem;
...
Notes:
- Tailwind v4 expects `--spacing-*` (with `ing`) for the spacing utilities. We mirror Burnish's `--space-*` to `--spacing-*` so both names work.
...
The translation from Burnish tokens:
...
:root {
  ...
  --accent:               58%   0.16  60;     /* accent (same as primary in Burnish) */
...
Then the user's `tailwind.config.ts` (or v4 `@theme`) reads them with `oklch(var(--background))`. Burnish's accent → shadcn's primary is the canonical mapping; secondary → paper-3.

If the user wants a dark variant, mirror with the dark theme tokens under a `.dark` selector (Burnish's Midnight / Bloom / Terminal supply the values).
...
4. **Merge into an existing entry stylesheet; never overwrite it.** When the target project already has `app/globals.css` (or `src/index.css`, `src/styles/global.css`): keep its existing `@import "tailwindcss"` / `@tailwind base|components|utilities` directives exactly as they are, append Burnish's `:root` tokens and base rules *after* them, and keep any `@import "tokens.css"` at the very top of the file (CSS parses `@import` only before other rules, so a misplaced one is silently dropped and your tokens vanish). If the project already defines brand tokens (`--background`, `--foreground`, a Tailwind `@theme`), map Burnish's roles onto those names rather than adding a parallel set, or scope Burnish's tokens under a wrapper class. Replace the file outright only when the user has asked for a full takeover.
