...
| **F** | **Variety** | Does this output share a structural fingerprint with a previous Burnish output in the project? Score by structural distance, not visual distance — colour-swaps don't count as variety. |

Record the six scores in a one-line stamp comment at the top of the file: `/* Burnish · pre-emit critique: P5 H4 E5 S4 R5 V5 */`. Future runs should be able to find this and avoid repeating the same weakness.
...
20. Is the `/* Burnish · macrostructure: <name> · ... */` stamp missing from the top of the CSS? (It must be present.)
