...
**Diversification rule:** within a single page, no two sections should use the same archetype. A Bento Grid might pair *Bento feature block* with *Inline form CTA* with *Logo wall (hairline)*. The next page Burnish builds should pick different archetypes from the same categories.
...
```css
/* Burnish · macrostructure: Bento Grid · F1 Bento knobs: tiles=6, spans=irregular, accent=corner-only · ... */
```
...
**Diversification.** Same rule as nav — across consecutive Burnish runs in the same session, no two outputs should share the same footer archetype.
...
The goal is composed variety — within a page, sections feel different from each other; across pages Burnish builds, sections feel different from the last.
