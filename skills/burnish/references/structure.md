...
Most AI-generated UIs are visually distinct but structurally identical: hero → three features → CTA → footer. Same heading positions, same column counts, same component vocabulary. **Structural sameness is the AI fingerprint, not visual sameness.** Burnish's job is to break it.
...
- **Hairline rule.** 0.5–1px line, inset or full-bleed. Burnish's default; modernist.
...
- **Outlined.** Border, no fill. Secondary or quiet primary. Burnish default.
...
Each Burnish theme has a default structural fingerprint. Use them as starting points only when the brief specifies a theme. **For most builds, pick a macrostructure from [`macrostructures.md`](macrostructures.md) instead** — themes describe *visual surface*, macrostructures describe *page shape*; the latter drives variety more.
...
If the user shrugs and says "you pick", read the project's CSS for a `/* Burnish · macrostructure: ... */` stamp; whichever of the trio is most categorically distant from the stamped family is the right pick. Two consecutive outputs should never be from the same family — never two editorial macrostructures, never two grid-led macrostructures.
