...
Loaded only when the user has opted into the **custom** theme route in Step 1 of the Design flow. Custom is **made-to-measure for one brief**, written inline into the page's `:root`, never a permanent catalog entry. It spans a **spectrum of depth**: at its lightest, a complete OKLCH palette + free-font pairing tuned to the brief while keeping Burnish's structures (the *combination* is per-brief); at its fullest — **bespoke** — the page's *structure and composition* are designed from first principles too, bound to no catalog theme, genre, or macrostructure. One route, chosen depth.
...
- **catalog** — the named-theme catalogue. Burnish's 21 themes (Specimen, Midnight, Brutal, Garden, Atelier, Newsprint, Terminal, Manifesto, Almanac, Sport, Studio, Riso, Bloom, Coral, Cobalt, Aurora, Editorial, Carnival, Lumen, Hum, Grid). Each one is a fixed combination of paper-band, display-style, and accent-hue. The rotation rule cycles through them so two consecutive runs don't read alike. **This is the default.** Most briefs use it.
- **custom** — made-to-measure, at the depth the brief needs:
  - **Tuned** — a one-off OKLCH palette + font pairing built for one brief, *keeping* Burnish's structures, archetypes, and macrostructures. The rules (paper L bands, accent chroma caps, font ban list, all slop-test gates) still apply; only the *combination* is per-brief.
...
Burnish must **not** offer catalog-vs-custom on every prompt. That's friction, not discipline. Surface the fork only when the brief carries one of these signals:
...
Most custom runs are *tuned* (a palette + pairing on Burnish's existing structures). **Bespoke** is the deep end, fired by signal 5: the brief's *structure itself* is the ask and no catalog shape fits. At this depth custom designs the page from first principles — palette, type, **and** composition — and the only thing it inherits is the floor.
...
**Stamp (bespoke runs):**
```css
/* Burnish · route: custom (bespoke) · structure: <one-line shape> · idea: "<central move>"
...
Write all three into the macrostructure stamp (§ E below) and the `.burnish/log.json` entry (§ F below). They are the durable record. The next run reads them.
...
```css
/* Burnish · macrostructure: <name> · <hero archetype + knobs>
...
```css
/* Burnish · macrostructure: Long Document · H5 hero knobs: salutation=time-stamp, body=2 paragraphs, signoff=initials
...
## § F · `.burnish/log.json` entry shape
...
**Stamp:**
```css
/* Burnish · macrostructure: Workbench · F2 sticky-scroll knobs: pinned=right, content=trace-panel, steps=3
...
**Stamp:**
```css
/* Burnish · macrostructure: Catalogue · F1 catalogue knobs: tiles=8, columns=2, rule=hairline-between
